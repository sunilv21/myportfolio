  ...
  return (
    <section id="portfolio" className="py-20 sm:py-28 px-4 sm:px-6 relative">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <motion.span
            className="text-[11px] uppercase tracking-[0.25em] text-orange-400/60 font-medium mb-4 block"
            initial={{ opacity: 0 }}
          ...
          <AnimatePresence mode="popLayout">
            {filteredContent.map((item, idx) => (
              <motion.div
                ...
                className="group"
              >
                <div className="h-full rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm overflow-hidden hover:border-orange-400/20 transition-all duration-500 hover:shadow-2xl hover:shadow-orange-500/[0.05]">
                  {/* Thumbnail */}
                  <div className="relative h-44 sm:h-48 overflow-hidden">
                    {item.thumbnail_url ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-orange-500/[0.06] to-transparent">
                        <span className="text-4xl mb-2 opacity-60">{item.categories?.icon || "📁"}</span>
                        <span className="text-[11px] text-white/20 font-medium tracking-wide">{item.categories?.name}</span>
                      </div>
                    )}
